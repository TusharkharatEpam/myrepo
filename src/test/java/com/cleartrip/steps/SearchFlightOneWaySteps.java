...
public class SearchFlightOneWaySteps{
	...
	public void init() throws IOException {
		...
		//getting property from JRE runtime
		if("chrome".equalsIgnoreCase(System.getProperty("browser")))
	...
	public void whenEntersTheDepartureDate(@Named("departDate") String departDate,
			@Named("returnDate") String returnDate,
			@Named("tripType") String tripType) {
		searchFlightPage.enterFlightDate(departDate,returnDate,tripType);
	}

	@When("user enter flight $source and $destination")
	public void whenUserEnterFlightSourceAndDestination(@Named("source") String source,@Named("destination") String destination) {
	...
	public void whenEntersDetailsOfAdultChildrenAndInfants(@Named("adults") String adult,
			@Named("children") String children,
			@Named("infants") String infants) {
	...
	public void givenUserOpenTheCleartripPortal() {
		...
		searchFlightPage.loginToPortal(env.getProperty("webURL"));
	}
